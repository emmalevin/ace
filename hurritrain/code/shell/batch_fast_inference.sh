#!/bin/bash
#SBATCH --job-name=ace_inf_pair
#SBATCH --output=/scratch/gpfs/GVECCHI/el2358/ace/hurritrain/slurm/inf_%x_%A.out
#SBATCH --error=/scratch/gpfs/GVECCHI/el2358/ace/hurritrain/slurm/inf_%x_%A.err
#SBATCH --nodes=1
#SBATCH --ntasks=2
...
: "${YAML_FILE_MODEL1:?Set YAML_FILE_MODEL1 to the model1 inference YAML path}"
: "${YAML_FILE_MODEL2:?Set YAML_FILE_MODEL2 to the model2 inference YAML path}"

NPROC_PER_MODEL="${NPROC_PER_MODEL:-1}"
PROFILE_MODEL1="${PROFILE_MODEL1:-0}"
PROFILE_MODEL2="${PROFILE_MODEL2:-0}"
PROFILE_OUTPUT_PREFIX="${PROFILE_OUTPUT_PREFIX:-timeline_output_inference}"
...
if (( ${#ALLOCATED_GPUS[@]} < REQUIRED_GPUS )); then
    echo "Need ${REQUIRED_GPUS} GPUs for paired inference, but Slurm exposed ${#ALLOCATED_GPUS[@]}: ${ALLOCATED_CUDA_VISIBLE_DEVICES:-unset}" >&2
    exit 1
fi
...
run_model() {
    ...
    local cmd=(
        ...
        -m fme.ace.batched_evaluator
        "${yaml_file}"
    )

    echo "Launching model${model_id} inference on CUDA_VISIBLE_DEVICES=${gpu_ids}"
...
if (( status_model1 != 0 || status_model2 != 0 )); then
    echo "Inference failed: model1 exit=${status_model1}, model2 exit=${status_model2}" >&2
    exit 1
fi

echo "Both inference models completed successfully."
